            ...
            <table id="bookings_table" class="table table-bordered text-dark">
                ...
                <tbody>
                    ...
                    <tr class='align-middle'>
                        <td class='fs-5 text-center' nowrap>
                            <a target="_blank" href="{{ route('user.view.invoice', $booking->id) }}"
                                class='text-info text-decoration-underline'>{{ $booking->invoices->invoice_number }}</a>
                        </td>
                        <td nowrap class='fs-5 text-center'>{{ $booking->room->name }}</td>
                        <td nowrap class='fs-5 text-center'>{{ $booking->room->roomType->type_name }}</td>
                        <td nowrap class='fs-5 text-center'>{{ $booking->room->capacity }}</td>
                        <td nowrap class='fs-5 text-center'>{{ $booking->total_price }}</td>
                        ...
                        <td class='text-center fs-5'>
                            <a href="{{ route('payment.success', $booking->id) }}" class='btn btn-info'>PRINT</a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
